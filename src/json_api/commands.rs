...
impl JsonApiCommands {
    pub async fn process(
        ...
        if input == "help" {
            return JsonApiCommands::result(format!("Valid commands: {}",
                permissions
                    .into_iter()
        ...
        if !permissions.contains(&perm) {
            return JsonApiCommands::result(format!("Error: Missing {} permission.", perm).to_string())
        }
        ...
        match parsed {
            Ok(cli) => {
                match coord.handle_console_cmd(cli.cmd).await {
                    Ok(res) => {
                        tracing::info!("{}", res);
            ...
            _ => {
                ...
                return JsonApiCommands::result(format!("Error: Invalid Command - {}", input.trim()).to_string())
            }
        }
    }

    pub fn result(str: String) -> JsonApiCommands {
        return JsonApiCommands {
            output : Some(str),
        }
    }
}
